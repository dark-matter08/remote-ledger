        ...
        <div className="panel autopilot">
          ...
          {progress && !progress.live && (
            <div className="autopilot-done">
              ...
              {progress.belowMinimum && (
                <div className="autopilot-blocked">
                  <p className="hint">
                    This posting scored <strong>{progress.belowMinimum.score}</strong>, under the
                    minimum of <strong>{progress.belowMinimum.minimum}</strong> you set in Settings
                    &rarr; Scheduler. Nothing after the match was run, so nothing was spent on it.
                  </p>
                  <autopilot.Form method="post" action="/api/autopilot">
                    <input type="hidden" name="jobId" value={job.id} />
                    <input type="hidden" name="force" value="1" />
                    <button className="btn ghost" disabled={autoLive}>Apply anyway</button>
                  </autopilot.Form>
                </div>
              )}
              {progress.ok && (
