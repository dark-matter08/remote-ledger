...
async function runSync(runId: number, acct: EmailAccount): Promise<void> {
  ...
  // Say what is answering, on the run itself. The crawl and the knowledge scans
  // already do this; email did not, so its rows read "—" until the next server start,
  // when the boot-time backfill guessed it from llm_calls inside the run's window. A
  // finished run should not have to wait for a restart to say what read the mail.
